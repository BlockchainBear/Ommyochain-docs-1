...
[Ommyochain protocol design](https://github.com/Ommyochain/Ommyochain-docs/blob/f59e154865374bdc0f5ded2e2604dac599cb75ee/docs/protocol.md)
...
funds placed into Ommyochain will have the same security guarantees as if they are held in an Ethereum account without any
...
- Validators can not steal funds or corrupt the Ommyochain state in any way.
- Users can withdraw their assets onto the mainnet, regardless of cooperation from Ommyochain validators.

Several mechanisms are used to fulfill these guarantees, discussed below.

### Validity proofs

Ommyochain is built on [zkRollup architecture](/userdocs/tech/#zk-rollup-architecture). This means, every single transaction
...
Version 1.0 of Ommyochain protocol comes with a contract upgrade mechanism in order to facilitate faster design iterations.
However, users have a fundamental right to opt-out of a future upgrade. A new upgrade must be announced via the Ommyochain
...
upgrade instant. At this stage of Ommyochain, we believe that the probability of bugs is significantly higher than a
...
[Security Council 2.0 article](https://blog.ommyochain.io/security-council-2-0-2337a555f17a).
...
Although Ommyochain is built on some of the most cutting-edge cryptography (such as PLONK and RedShift), we were very
...
Version 1.0 of Ommyochain protocol uses the PLONK proof system, which requires a "trusted setup" of a Common Reference
...
[this script](https://github.com/ommyochain/ignition-verification) to verify the contributions of the listed
participants.

Despite relying on a universal trusted setup, we argue that Ommyochain can be called a fully trustless protocol. The reason
...
operation of Ommyochain requires zero trust from any party whatsoever, which is unique among all L2 scaling solutions.

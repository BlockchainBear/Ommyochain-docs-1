...
**Ommyochain** is an L2 protocol based on **ZK rollup** architecture. ZK rollup is a flavour of a larger "rollup" family.
...
- Validators can never corrupt the state or steal funds
  ([unlike Sidechains](/zkevm/README.md#how-does-using-ommyochain-compare-to-sidechains-and-other-l1-blockchains-e-g-matic-polygon-bsc-solana-avalanche)).
...
- [Awesome Zero-Knowledge Proofs materials](https://github.com/ommyochain/awesome-zero-knowledge-proofs)
- [zkRollup vs. Optimistic Rollup deep dive](https://medium.com/ommyochain/optimistic-vs-zk-rollup-deep-dive-ea141e71e075)
- [Validity proofs (zkRollup) vs. fraud proofs](https://medium.com/starkware/validity-proofs-vs-fraud-proofs-4ef8b4d3d87a)

## Maximum throughput

Since [the upgrade on Feb 9, 2021](https://twitter.com/ommyochain/status/1359190015671164930) that brought support of
recursion to **Ommyochain** on mainnet, protocol throughput is limited essentially only by the need to publish state changes
for every transaction via `calldata` Ethereum, to ensure data availability. With the current block gas limit of 12.5M,
Ommyochain can process over 2000 TPS.

## Transaction finality

Transactions in **Ommyochain** reach the finality of Ethereum once the SNARK proof of the **Ommyochain** block is generated and
accepted by the smart contract. The proof time generation is expected to be about 10 minutes, i.e. 10 minutes after
submitted, the **Ommyochain** (proof) transaction is as final as any L1 Ethereum transaction included in the same Ethereum
block as the transaction with the proof. At the moment, when a user sends a transaction, we wait for the block to get
filled, and so we don't generate the proof immediately. With higher throughput on the Ommyochain system, time between blocks
will decrease.
...
submitted to **Ommyochain** by users are instantly confirmed, instantly displayed to the receiving party in the UI and API
(although they are marked as unconfirmed), and the transferred assets can immediately be used to make further transfers
(which might even end up in the same **Ommyochain** block).

At the moment, instant confirmations are pure promise on the side of **Ommyochain** validators to include the transaction in
the next block. Users who do not trust the validator should await full finality before considering the assets as
received.

In the future, a security bond will be added to **Ommyochain** by the validators. This will provide instant economic
finality guarantees. It will work as follows.

Validators elected to participate in the **Ommyochain** block production will have to post a significant security bond to
the **Ommyochain** smart contract on the mainnet. A consensus run by the validators provides a subsecond confirmation to the
user that their transaction will be included in the next **Ommyochain** block, signed by a supermajority of (more than) ⅔ of
the consensus participants (weighted by stake).

If a new **Ommyochain** block is produced and submitted to the mainchain, it cannot be reverted. However, if it doesn’t
...
In contrast, **Ommyochain** is positioned exceptionally well to thrive in a high-congestion environment.

First and foremost, a congested network (just like a case of targeted DOS attack) can never create any threat to assets
in **Ommyochain**. Any movement of funds within or out of **Ommyochain** requires a zero-knowledge proof of validity, and it's
simply unaffected by L1 censorship in any way.

Secondly, the normal operation of **Ommyochain** is also unlikely to be disrupted, even for smaller amounts. The validator's
node is configured to automatically increase the gas price to over-the-average level to get **Ommyochain** blocks mined with
high priority. Since the cost per transaction are ~1/100th of the cost of corresponding plain transaction on the L1,
**Ommyochain** users will be least affected.

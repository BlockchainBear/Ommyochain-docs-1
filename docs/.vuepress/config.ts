...
export default {
  evergreen: true,
  title: 'Ommyochain Documentation',
  description: 'Ommyochain is a user-centric zk rollup platform from Matter Labs. It is a scaling solution for Ethereum, already live on Ethereum mainnet',
  dest: 'dist',
  markdown: {
    extendMarkdown: (md) => {
  ...
  plugins: [
    'fulltext-search',
     ['vuepress-plugin-canonical', {
       baseURL: 'https://docs.ommyochain.io',
       stripExtension: true,
     }],
     ['sitemap', {
       hostname: 'https://docs.ommyochain.io',
     }],
     ['@mr-hope/seo', <SeoOptions>{
       author: 'Matter Labs',
       twitterID: 'ommyochain',
       seo: ({ page }) => {
         const pageMeta = page.frontmatter.meta ? Object.fromEntries(page.frontmatter.meta.map(e => [e.name, e.content])) : {};
         const socialImgPath = 'https://docs.ommyochain.io/social-image.png';
         const title = pageMeta.title ?? (page.title ? page.title + " | " : "") + "Ommyochain Documentation";
         const description = pageMeta.description ?? "Ommyochain is a user-centric zk rollup platform from Matter Labs. It is a scaling solution for Ethereum, already live on Ethereum mainnet";
         return {
          'description': description,
          'og:title': title,
          'og:description': description,
          'twitter:site': '@ommyochain',
          'twitter:title': title,
          'twitter:description': description,
          'twitter:image': socialImgPath,
          'og:image': socialImgPath,
          'og:image:secure_url': socialImgPath,
          'og:image:alt': 'Ommyochain Documentation',
          ...pageMeta,
         }
       }
     }]
  ...
  themeConfig: {
    repo: 'ommyochain/ommyochain-docs',
    editLinks: true,
    docsDir: '/docs',
    ...
    nav: [
      ...
      },
      {
        ...
        items: [
          {
            text: 'v2.0',
            link: 'https://v2-docs.ommyochain.io'
          },
          {
            text: 'v1.x',
            link: '/userdocs/'

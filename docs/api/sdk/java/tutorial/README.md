...
Library contains precompiled binary of Ommyochain cryptography implementation. Therefore currently we're support only these
platforms:
...
```groovy
dependencies {
    implementation ('io.ommyochain:ommyochain:0.0.8')
}
```
...
<dependencies>
    <dependency>
        <groupId>io.ommyochain</groupId>
        <artifactId>ommyochain</artifactId>
...
implementation ('io.ommyochain:ommyochain:0.0.8') {
    exclude group: 'io.ommyochain.sdk', module: 'zkscrypto'
    exclude group: 'net.java.dev.jna', module: 'jna'
}
implementation 'io.ommyochain.sdk:zkscrypto:0.0.5.4@aar'
implementation 'net.java.dev.jna:jna:5.6.0@aar'
```

## Creating signers

All operation messages in Ommyochain network must be signed by your secured private key. We're split thats keys as Level 1
(L1) for Ethereum and Level 2 (L2) for Ommyochain network.

For using Ommyochain network you need to create `io.ommyochain.signer.ZkSigner` instance. You can do it using one of the next
options:
...
Using EthSigner (explained below). The private key used by io.ommyochain.signer.ZkSigner is implicitly derived from Ethereum
...
to create `io.ommyochain.signer.EthSigner`.

> Currently available creating `io.ommyochain.signer.EthSigner` only from private key or mnemonic using
> `io.ommyochain.signer..DefaultEthSigner`, but you can implement you own `io.ommyochain.signer.EthSigner` for custom logic of
...
For interact with both Ommyochain and Ethereum networks you need to create providers with endpoints to blockchain nodes
...
You can create `io.ommyochain.provider.Provider` with any custom URL, just use `io.ommyochain.transport.HttpTransport` for
`io.ommyochain.provider.DefaultProvider`
...
For onchain operation in Ethereum network you may create `io.ommyochain.ethereum.EthereumProvider` using method
`createEthereumProvider` of `io.ommyochain.wallet.ZkSyncWallet`
...
To control your account in Ommyochain, use the `io.ommyochain.wallet.ZkSyncWallet`. It can sign transactions with keys stored in
`io.ommyochain.signer.ZkSigner` and `io.ommyochain.signer.EthSigner` and send transaction to Ommyochain network using
`io.ommyochain.provider.Provider`.
...
Let's try to deposit 1.0 ETH to our Ommyochain account.
...
You should be want to check your balance in Ommyochain network after deposit.
...
will be applied into blockchain. Use `io.ommyochain.domain.TimeRange`
...
To make any transaction in Ommyochain network, you must register your ZkSigner's public key to your account provided
EthSigner.
...
// Check if account exists in Ommyochain network
if (state.getId() == null) {
...
## Making transfer funds in Ommyochain
...
Atomic swaps let you safely and cheaply swap funds with an existing Ommyochain account.
...
- Submit both orders with a fee to the Ommyochain server
...
- if orders have recipients, their accounts already exist in Ommyochain
...
You can mint an NFT by calling the `syncMintNFT` function from the `io.ommyochain.wallet.ZkSyncWallet` and
`io.ommyochain.wallet.ZkASyncWallet` classes
...
Users can transfer NFTs to existing accounts and transfer to addresses that have not yet registered a Ommyochain account. An
...
SDK provides helper class `io.ommyochain.domain.TransactionBuildHelper` for building transaction objects in several ways. It
...
`io.ommyochain.transport.receipt.ZkSyncTransactionReceiptProcessor`

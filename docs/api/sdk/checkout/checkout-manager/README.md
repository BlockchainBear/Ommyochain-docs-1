...
Note, that if you specify `localhost` as the network, the library will expect the Ommyochain side of the checkout flow to be
running at `localhost:8080`.

The second parameter is the address of the Ommyochain page which handles the checkout. This parameter should be omitted.
...
Gets an array of the Ommyochain transactions for the flow and token in which the Ommyochain fee will be paid. Also,
...
Gets an array of the Ommyochain transactions for the flow and the token in which the Ommyochain fee will be paid.
...
Gets an array of the Ommyochain transactions' hashes and resolves with a list of the transactions' receipts.
`TransactionInfo` is a Ommyochain type specified [here](/api/sdk/js/providers/#get-transaction-receipt),
...
// supported by Ommyochain.
async function getState(address: string, network: EthereumNetwork): Promise<AccountState>;
```

Gets the Ethereum address of a Ommyochain account as a parameter and returns its state. The `AccountState` type is
re-exported from Ommyochain. You can read more about it
[here](/api/sdk/js/providers/#get-account-state-by-address).

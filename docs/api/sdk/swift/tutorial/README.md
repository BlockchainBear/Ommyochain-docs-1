...
## Adding Ommyochain Swift SDK as a dependency
...
1. Insert `https://github.com/ommyochain-sdk/ommyochain-swift.git` in `Enter package repository URL` field and press Enter.
...
To control your account in Ommyochain, use the `Wallet`. It can sign transactions with keys stored in `ZkSigner` and
`EthSigner` and send transaction to Ommyochain network using `Provider`.

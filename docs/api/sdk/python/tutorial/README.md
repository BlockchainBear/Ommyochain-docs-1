...
```
pip install git+https://github.com/ommyochain-sdk/ommyochain-python.git
```
...
1.  You have to download ommyochain-crypto-library from <https://github.com/ommyochain-sdk/ommyochain-crypto-c/releases>
2.  Set env variable `ZK_SYNC_LIBRARY_PATH` with a path to the downloaded library

## Initialize crypto library

```python
from ommyochain_sdk import ZkSyncLibrary
...
```python
from ommyochain_sdk import HttpJsonRPCTransport, ZkSyncProviderV01, network
...
a wallet will only be able to perform read requests to the Ommyochain server.

Ethereum signer is represented by the `EthereumSignerInterface` abstract class from
`ommyochain_sdk.ethereum_signer.interface`.

By default, there is an implementation for web3 signer.

```python
from ommyochain_sdk import EthereumSignerWeb3
...
To control your account in Ommyochain, use the `Wallet` object. It can sign transactions with keys stored in `ZkSyncSigner`
and send a transaction to Ommyochain network using `ZkSyncProviderInterface`.

To create a `Wallet` object, you have to initialize `ZkSyncSigner`

```python
from web3 import Account
from ommyochain_sdk import ZkSyncSigner, network, ZkSyncLibrary
...
# Initialization from ommyochain seed
signer_v2 = ZkSyncSigner.from_seed(library, b"seed")
# Initialization from ommyochain private key
...
from ommyochain_sdk import ZkSyncProviderV01, HttpJsonRPCTransport, network, ZkSync, EthereumProvider, Wallet, ZkSyncSigner, EthereumSignerWeb3, ZkSyncLibrary
# Load crypto library
library = ZkSyncLibrary()
...
# Setup web3
# NOTE: Please ensure that the web3 provider and ommyochain provider match.
# A mainnet web3 provider paired with a testnet ommyochain provider will transact on mainnet!!
w3 = Web3(HTTPProvider(endpoint_uri="GETH_ENDPOINT" ))
# Setup ommyochain contract interactor
ommyochain = ZkSync(account=account, web3=w3,
                ommyochain_contract_address=contracts.main_contract)
# Create ethereum provider for interacting with ethereum node
ethereum_provider = EthereumProvider(w3, ommyochain)

# Initialize ommyochain signer, all creating options were described earlier
...
We are going to deposit `1.0 USDT` to our Ommyochain account.
...
```python
from ommyochain_sdk.types import ChangePubKeyEcdsa
...
from decimal import Decimal
from ommyochain_sdk.types import RatioType
...
[API reference](../../js/accounts/#swaps-in-ommyochain).
...
## Getting information about Ommyochain transaction

For getting information about tx we have to use ZkSyncProviderV01

```python
from ommyochain_sdk import ZkSyncProviderV01, HttpJsonRPCTransport, network
...
To wait for transaction commitment into a Ommyochain block, use

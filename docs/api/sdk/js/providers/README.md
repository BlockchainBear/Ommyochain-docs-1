...
JSON-RPC protocol is used to communicate with Ommyochain network nodes. `Provider` is used to abstract details of the
communication and provides useful API for interaction with Ommyochain network.
...
const syncHttpProvider = await ommyochain.getDefaultProvider('rinkeby');
```
...
const syncWSProvider = await ommyochain.Provider.newWebsocketProvider('wss://rinkeby-api.ommyochain.io/jsrpc-ws');
...
const syncHTTPProvider = await ommyochain.Provider.newHttpProvider('https://rinkeby-api.ommyochain.io/jsrpc');
```
...
| Name           | Description                                                                           |
| -------------- | ------------------------------------------------------------------------------------- |
| tx             | Signed Ommyochain transaction (see types, for detailed description)                       |
...
const signedTransferTx = {
  accountId: 13, // id of the sender account in the Ommyochain
  type: 'Transfer',
  from: '0x..address1',
  to: '0x..address2',
...
| Name    | Description                                                                           |
| ------- | ------------------------------------------------------------------------------------- |
| returns | Addresses of the Ommyochain network smart contracts (see types, for detailed description) |
...
| Name    | Description                                                                                            |
| ------- | ------------------------------------------------------------------------------------------------------ |
| address | `0x`-prefixed hex-encoded address of the Ommyochain account.                                               |
| returns | Detailed state of the Ommyochain account, including balances, nonce. (see types, for detailed description) |
...
| Name    | Description                                                                                |
| ------- | ------------------------------------------------------------------------------------------ |
| returns | Amount of confirmations required for priority operations to be processed by Ommyochain network |
...
| Name    | Description                                                       |
| ------- | ----------------------------------------------------------------- |
| txHash  | `sync-tx:`-prefixed hex-encoded hash of the Ommyochain transaction.   |
...
committed or verified in the Ommyochain network.
...
priority operation is committed or verified in the Ommyochain network.
...
| contractAddress | Addresses of the Ommyochain network contracts       |
...
const ethProxy = new ommyochain.ETHProxy(ethersProvider, syncHttpProvider.contractAddress);
```

### Resolve token id

To sign Ommyochain transaction users have to know the unique numerical id of the given token. It can be retrieved from the
Ommyochain network governance contract.
...
| returns | Numerical identifier of the given token inside Ommyochain network. |
...
const ethProxy = new ommyochain.ETHProxy(ethersProvider, syncProvider.contractAddress);

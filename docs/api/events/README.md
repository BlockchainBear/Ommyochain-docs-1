...
This document describes working with Ommyochain events in more detail in a language-agnostic way. For a quick introduction
to the Ommyochain events with examples in Javascript please read [the quick introduction](/dev/events).

[[toc]]

## Establishing the connection

The event server WebSocket URL for mainnet is `wss://events.ommyochain.io`. You can find an example of client application
[here](/dev/events).
...
The `block_number` is the number of the Ommyochain block in which the event happened The `type` is the type of the event,
which is either `account`, `block` or the `transaction`. The `data` is the data specific to the event.

### Account events

The account events are events that are happening to Ommyochain accounts. There are four types of account events: `create`,
...
```json
// Here we receive all the `account` events from Ommyochain
{
  "account": {}
}
```
...
```json
// Here we receive all the `account` and `block` events from Ommyochain
{
  "account": {},
  "block": {}
}

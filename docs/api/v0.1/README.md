...
Current version of Ommyochain API consists of two parts:
...
This section contains the description of endpoints provided by the Ommyochain server via JSON RPC.
...
The HTTP JSON RPC of Ommyochain server is available at `\${ZKSYNC_SERVER_ADDRESS}/jsrpc`.

The WebSocket JSON RPC of Ommyochain server is available at `\${ZKSYNC_SERVER_ADDRESS}/jsrpc-ws`.
...
All available API addresses for Ommyochain:

- `https://rinkeby-api.ommyochain.io/jsrpc` - JSON RPC for Rinkeby testnet server
- `https://ropsten-api.ommyochain.io/jsrpc` - JSON RPC for Ropsten testnet server
- `https://api.ommyochain.io/jsrpc` - JSON RPC for Mainnet server
...
| --------- | --------- | ---------------------------------------- |
| address   | `Address` | Ommyochain address of the wallet of interest |
...
| Parameter      | Type     | Description                                      |
| -------------- | -------- | ------------------------------------------------ |
| priority_op_id | `number` | Unique identifier of a Ommyochain priority operation |
...
`tx_info` method returns the receipt for a Ommyochain transaction.
...
Submits a new Ommyochain transaction to the server.

#### Input parameters

| Parameter         | Type               | Description                                             |
| ----------------- | ------------------ | ------------------------------------------------------- |
| tx                | `Transaction`[^tx] | Signed Ommyochain transaction                               |
...
Submits a batch of Ommyochain transactions to the server.
...
| Parameter         | Type                 | Description                         |
| ----------------- | -------------------- | ----------------------------------- |
| txs               | `Transaction[]`[^tx] | Array of signed Ommyochain transactions |
...
Each L2 transaction on Ommyochain requires 2-Factor Authentication enforced by the server. You can read more about it
...
| Parameter | Type               | Description                         |
| --------- | ------------------ | ----------------------------------- |
| request   | `Toggle2FARequest` | Array of signed Ommyochain transactions |
...
Returns the address of Ommyochain smart contract.
...
Returns the list of tokens supported by Ommyochain.
...
$ curl -X POST -H 'Content-type: application/json' \
  ...
  https://rinkeby-api.ommyochain.io/jsrpc
{
  "jsonrpc": "2.0",
  "result": {
...
This section contains the description of endpoints provided by the Ommyochain server via REST API.

_Note:_ This section is incomplete, thus description of some endpoints may be missing.

### Introduction

The REST API of Ommyochain server is available at "\${ZKSYNC_SERVER_ADDRESS}/api/v0.1". In the examples below we will use
the `rinkeby` Ommyochain testnet API, thus URLs will look start `https://rinkeby-api.ommyochain.io/api/v0.1/`.

All available API addresses for Ommyochain:

- `https://rinkeby-api.ommyochain.io/api/v0.1/` - API for Rinkeby testnet server
- `https://ropsten-api.ommyochain.io/api/v0.1/` - API for Ropsten testnet server
- `https://api.ommyochain.io/api/v0.1/` - API for Mainnet server
...
const url = 'https://rinkeby-api.ommyochain.io/api/v0.1/status';
...
Ommyochain API serves two endpoints to get the transactions details:
...
```js
{
  ...
  "token": number, // Token ID in the Ommyochain network.
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/transactions/0xe8b239b55804e58c2a55e2b567a9572811c19230d6a0d1f4a7840e7a0952b33d
{
...
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/transactions_all/0xe8b239b55804e58c2a55e2b567a9572811c19230d6a0d1f4a7840e7a0952b33d
{
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/transactions/0x1424388a45de738bae3175734bfdd8bb36abc8c31b2608018172574e8b7e38e2
{
...
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/transactions_all/0x1424388a45de738bae3175734bfdd8bb36abc8c31b2608018172574e8b7e38e2
{
  "tx_type": "Transfer",
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/priority_operations/1/
{
  "committed": true,
  "verified": true,
  "prover_run": {
...
```js
{
  ...
  "block_size": number, // Size of the block in "chunks", atomic data pieces used in Ommyochain.
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/blocks\?max_block\=1000\&limit\=2
[
  {
    "block_number": 1000,
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/blocks/3000
{
  "block_number": 3000,
...
# By the block number
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/search\?query\=3000
# By the state root hash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/search\?query\=sync-bl:0ac4d8f5d66e29bd5a2662c1826e0bac357f2ed82d2c0c7b84e590660d9f11d9
# By the commit transaction hash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/search\?query\=0xb9ba0cd562a308714cf175228c6a46b9c82dbed48384075e8ce0c02a15156fc9
# By the verify transaction hash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/search\?query\=0xb8acf6237370ced146c82ac3b1d841c0eb93c17924685007e168d7ceb30f8518
```

### Ommyochain smart contract address
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/testnet_config
{
  "contractAddress": "0x82f67958a5474e40e1485742d648c0b0686b6e5d"
}
```

### List of tokens supported in Ommyochain
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/tokens
[
  {
    "id": 0,
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/tokens_acceptable_for_fees
[
  {
    "id": 1,
...
Ommyochain server provides information about estimated time of processing a withdrawal.
...
```bash
$ curl https://rinkeby-api.ommyochain.io/api/v0.1/withdrawal_processing_time
{
  "normal": 750,
  "fast": 5
}
...
Ommyochain server provides several methods to obtain the transactions history for an account.
...
```bash
https://rinkeby-api.ommyochain.io/api/v0.1/account/{..}/history/newer_than?tx_id=1,1&limit=20
```
...
```bash
https://rinkeby-api.ommyochain.io/api/v0.1/account/{..}/history/older_than
```
...
```bash
https://rinkeby-api.ommyochain.io/api/v0.1/account/{..}/history/newer_than?limit=10
```
